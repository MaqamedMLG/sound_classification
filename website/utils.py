...
def spec_to_img(spec, eps=1e-6):
    ...
    if spec_max - spec_min < eps:
        spec_scaled = np.zeros_like(spec_norm, dtype=np.uint8)
    else:
...
def melspectrogram_db(fpath, sr=None, n_fft=2048, hop_length=512, n_mels=128, fmin=20, fmax=8300, top_db=80):
    '''
    extract mel spectrum decibel data from audio
    '''
    ...
    if wav.shape[0] < 5 * sr:
        ...
        wav = wav[:5 * sr]
    # fetch melspectrogram using librosa, then convert the spectrum to decibels
